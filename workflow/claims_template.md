...
Use this template when generating `expert_validation_claims.md` in the `agent-output/` folder.
The document structure, formatting, and field order must match exactly.

The Markdown file is the canonical claims document. A `.docx` is produced only on explicit curator request (see `annotate_gocam_outputs.md`).
